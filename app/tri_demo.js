...
const openVideoFromUser = (exports.openVideoFromUser = () => {
  ...
  console.log(
    'Server: user selected the video: ' + path.win32.basename(videoFile)
  );

  const OBD_file = path.join(app.getAppPath(), 'app/data/OBD.json');
  const infras_seg_result_file = path.join(
    app.getAppPath(),
    'app/data/infras_segmentation_result.json'
  );
  const traffic_seg_result_file = path.join(
    app.getAppPath(),
    'app/data/traffic_segmentation_result.json'
  );

  try {
    OBD_data = JSON.parse(fs.readFileSync(OBD_file, 'utf8'));
    OBD_data = OBD_data['data_samples'];
    infras_seg_result = JSON.parse(
      fs.readFileSync(infras_seg_result_file, 'utf8')
    );
    traffic_seg_result = JSON.parse(
      fs.readFileSync(traffic_seg_result_file, 'utf8')
    );
  } catch (err) {
  ...
  mainWindow.webContents.send(
    'opened-infras_segment_result',
    infras_seg_result
  );

  mainWindow.webContents.send(
    'opened-traffic_segment_result',
    traffic_seg_result
  );
...
const queryShapePoints = data => {
  db.all(
    'SELECT shape_points from link_to_shape_points where link_ID=' +
      data.link_ID,
    (err, row) => {
...
const updateMap = (exports.updateMap = reqTimeStamp => {
  ...
  let interval = 1;
  let realInterval = Math.abs(
    Math.floor(reqTimeStamp) - Math.floor(lastReqTimestamp)
  );
  ...
  if (realInterval >= interval) {
    ...
    // Update by 5s
    if (Math.floor(reqTimeStamp) % 5 == 0) updateRealtimeInfo(reqTimeStamp);
  }
});
...
const updateSegInfo = (exports.updateSegInfo = reqTimeStamp => {
  ...
  console.log(
    'segment infrastructure type: ' + cur_infras_seg.infras_type.toString()
  );
  ...
  console.log(
    'enter into a new traffic_segmentation: ' + cur_traffic_seg_index
  );
  console.log(
    'segment traffic level type: ' + cur_traffic_seg.traffic_type.toString()
  );
