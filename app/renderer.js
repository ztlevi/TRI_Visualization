...
$('#toggleFixedInterval').click(() => {
  if (fixedInterval == -1) fixedInterval = 60;
  else fixedInterval = -1;
...
function drawChart(data_index) {
  ...
  for (let i = 0; i < checkboxes.length; i++) {
    ...
    if (document.getElementById(id1).checked) dataset1.push(i + 2);
    let id2 = checkboxes[i] + '2';
    if (document.getElementById(id2).checked) dataset2.push(i + 2);
  }
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    for (let j = 2; j < realtimeInfo[i].length; j++) {
      if (j in realtimeInfoMax)
        realtimeInfoMax[j] = Math.max(
          realtimeInfoMax[j],
          Math.max(parseFloat(realtimeInfo[i][j]))
        );
      else realtimeInfoMax[j] = parseFloat(realtimeInfo[i][j]);
      if (j in realtimeInfoMin)
        realtimeInfoMin[j] = Math.min(
          realtimeInfoMin[j],
          Math.min(parseFloat(realtimeInfo[i][j]))
        );
      else realtimeInfoMin[j] = parseFloat(realtimeInfo[i][j]);
    }
  }
  ...
  for (let i = startIdx; i <= endIdx; i++) {
    ...
    for (let j = 0; j < dataset1.length; j++) {
      ...
      if (diff == 0) {
        diff = Math.max(
          Math.abs(realtimeInfoMax[idx]),
          Math.abs(realtimeInfoMin[idx])
        );
        if (diff == 0) diff = 1;
      }
...
const drawFrame = () => {
  ...
  if (framed && null !== cur_infras_seg_info) {
    ...
    let text =
      'Infras level: ' +
      cur_infras_seg_info.infras_type.toString() +
      '\nTraffic level: ' +
      cur_traffic_seg_info.traffic_type.toString();
...
ipcRenderer.on('opened-infras_segment_result', (event, infras_seg_result) => {
  ...
  $.each(infras_seg, function(i, infras_seg) {
    infras_select.append(
      $('<option>', {
        value: i,
        text:
          infras_seg.start_time +
          ' - ' +
          infras_seg.end_time +
          ' : ' +
          infras_seg.infras_type,
      })
    );
  });
  ...
  function checkInfrasSegEnd() {
    ...
    if (idx >= 0) {
      ...
      if (
        curSegStartTime != null &&
        Math.ceil(videoPlayer.currentTime) < curSegStartTime
      ) {
      ...
      if (
        curSegEndTime != null &&
        Math.floor(videoPlayer.currentTime) > curSegEndTime
      ) {
  ...
  $.each(traffic_seg, function(i, traffic_seg) {
    traffic_level_select.append(
      $('<option>', {
        value: i,
        text:
          traffic_seg.start_time +
          ' - ' +
          traffic_seg.end_time +
          ' : ' +
          traffic_seg.traffic_type,
      })
    );
  });
...
videoPlayer.addEventListener('timeupdate', () => {
  console.log(
    'event: video-player -> ontimeupdate\n' +
      'currentTime: ' +
...
ipcRenderer.on(
  'draw_seg_info',
  (event, curInfrasSegInfo, curTrafficSegInfo) => {
